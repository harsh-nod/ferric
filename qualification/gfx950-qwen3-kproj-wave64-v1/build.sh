...
package="$repo/device/gfx950-qwen3-kproj-wave64-v1"
...
mkdir -- "$out"
artifact="$out/qwen3-kproj-wave64-v1"
inputs=(
...
env PATH="$nightly/bin:/usr/local/bin:/usr/bin:/bin" \
  ...
  FE2O3_EXTRACT_CRATE_V1=ferric_gfx950_qwen3_kproj_wave64_v1 \
...
printf 'Built inert wave64 GEMV candidate: %s\n' "$artifact.hsaco"
printf 'No GPU execution, performance improvement or production authority is implied.\n'
