...
fn visible_body(function: &mut ItemFn) -> Result<&mut Block, &'static str> {
    let local = function.block.stmts.iter_mut().find_map(|statement| {
        let Stmt::Local(local) = statement else { return None };
        matches!(&local.pat, Pat::Tuple(pattern) if pattern.elems.len() == 2)
            .then_some(local)
    }).ok_or("missing recurrence binding")?;
    let Expr::Block(recurrence) = local.init.as_mut().ok_or("recurrence init")?.expr.as_mut()
        else { return Err("recurrence block") };
    let loop_body = recurrence.block.stmts.iter_mut().find_map(|statement| match statement {
        Stmt::Expr(Expr::While(value), _) => Some(value),
        _ => None,
    }).ok_or("missing token loop")?;
    let Some(Stmt::Expr(Expr::If(visible), _)) = loop_body.body.stmts.first_mut()
        else { return Err("visible-token branch") };
...
fn relocation_only(candidate: &str) -> Result<(), &'static str> {
    ...
    let insert_at = original.block.stmts.iter().position(|s| binding(s, "math"))
        .ok_or("baseline admission boundary")?;
    if !candidate.block.stmts.get(insert_at).is_some_and(|s| binding(s, "query_0"))
        || !candidate.block.stmts.get(insert_at + 1).is_some_and(|s| binding(s, "query_1"))
        || !candidate.block.stmts.get(insert_at + 2).is_some_and(|s| binding(s, "math"))
    {
        return Err("hoist boundary or order");
    }
    let moved: Vec<_> = candidate.block.stmts.drain(insert_at..insert_at + 2).collect();
    let old_body = visible_body(original)?;
    let old_at = old_body.stmts.iter().position(|s| binding(s, "query_0"))
...
fn source_mutations_cannot_change_admission_math_loads_or_output() {
    for (old, new) in [
        ("query_view.load_or(head_row, lane, 0)", "query_view.load_or(head_row, lane + 1, 0)"),
        ("query_view.load_or(head_row, lane + 64, 0)", "query_view.load_or(head_row, lane, 0)"),
        ("query_view.load_or(head_row, lane, 0)", "key_view.load_or(head_row, lane, 0)"),
        ("let query_0 =", "let wrong_query_0 ="),
        ("if position < max_context_tokens", "if position <= max_context_tokens"),
        ("physical_page < physical_pages", "physical_page <= physical_pages"),
        ("while token < max_context_tokens", "while token <= max_context_tokens"),
        ("token += 1", "token += 2"),
        ("query_0 * key_0", "query_1 * key_0"),
        ("product_0 + product_1", "product_0 - product_1"),
        ("reduce_sum_f32::<64>(partial)", "reduce_sum_f32::<32>(partial)"),
        ("finite &= product_0.is_finite()", "finite |= product_0.is_finite()"),
        ("math.exp_f32(maximum - next_maximum)", "math.exp_f32(score - next_maximum)"),
        ("numerator_0 / denominator", "numerator_1 / denominator"),
        ("max_grid = [1024, 1, 1]", "max_grid = [1025, 1, 1]"),
        ("lane + 64", "lane + 63"),
    ] {
        assert!(SOURCE.contains(old), "mutation must reach actual source: {old}");
        assert!(relocation_only(&SOURCE.replacen(old, new, 1)).is_err(), "{old}");
    }
    let query_0 = "    let query_0 = Bf16::from_bits(query_view.load_or(head_row, lane, 0)).to_f32();\n";
    let query_1 = "    let query_1 = Bf16::from_bits(query_view.load_or(head_row, lane + 64, 0)).to_f32();\n";
    let pair = format!("{query_0}{query_1}");
    assert!(SOURCE.contains(&pair));
    let early = SOURCE.replace(&pair, "").replace("    let query_base =", &(pair.clone() + "    let query_base ="));
...
fn single_root_and_six_slice_five_scalar_abi_remain_closed() {
    ...
    let widths: Vec<_> = function.sig.inputs.iter().map(|arg| {
        let FnArg::Typed(arg) = arg else { panic!("receiver") };
        match arg.ty.as_ref() {
            ...
            Type::Path(path) if path.path.segments.last().unwrap().ident == "WriteOnlyDisjointSlice" => 16,
...
fn standalone_manifest_keeps_exact_compiler_and_no_old_routes() {
    ...
    assert_eq!(manifest.matches("8efd4fd416d1ffae7a718144e4d299fe3c8f7590").count(), 2);
