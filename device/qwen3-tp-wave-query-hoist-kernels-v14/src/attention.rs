...
#[kernel(typed, launch(required = [64, 1, 1], max = [64, 1, 1], max_grid = [1024, 1, 1]), control_flow(loop_bounds(8192)))]
#[allow(clippy::too_many_arguments)]
pub fn ferric_qwen3_tp_batch32_wave_paged_gqa_query_hoist_bf16_v14(
    query: &[u16],
    key_cache: &[u16],
    value_cache: &[u16],
    positions: &[u32],
    ...
    if query.len() < head_rows * 128
        || query.len() > 32 * query_heads * 128
        || output.len() < head_rows * 128
        || output.len() > 32 * query_heads * 128
    ...
    let Ok(key_view) =
        StridedReadView2D::from_shared_slice(key_cache, 0, physical_pages * 16, columns, columns)
    else {
        fe2o3_device::trap();
    };
    let Ok(value_view) =
        StridedReadView2D::from_shared_slice(value_cache, 0, physical_pages * 16, columns, columns)
    else {
        fe2o3_device::trap();
    };
    ...
    let position =
        subgroup.broadcast_f32::<64>(position_view.load_or(row, 0, u32::MAX) as f32, 0) as usize;
    if position < max_context_tokens {
    } else {
        fe2o3_device::trap();
    }
    let query_base = head_row * 128;
    if query_base < 131072 {
    } else {
        fe2o3_device::trap();
    }
        ...
        while token < max_context_tokens {
            if token <= position {
                ...
                let physical_page = subgroup
                    .broadcast_f32::<64>(table_view.load_or(row, token / 16, u32::MAX) as f32, 0)
                    as usize;
                ...
                let key_1 =
                    Bf16::from_bits(key_view.load_or(cache_row, cache_column + 64, 0)).to_f32();
