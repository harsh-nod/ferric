...
macro_rules! four_dots {
    ($a:expr, $weights:expr, $rows:expr, $row_base:expr, $column:expr) => {{
        ...
        while inner < 1024 {
            let right =
                Bf16::from_bits(memory::volatile_load($weights, $column * 1024 + inner)).to_f32();
            if row_0 < $rows {
                let left =
                    Bf16::from_bits(memory::volatile_load($a, row_0 * 1024 + inner)).to_f32();
            ...
            if row_1 < $rows {
                let left =
                    Bf16::from_bits(memory::volatile_load($a, row_1 * 1024 + inner)).to_f32();
                let product = left * right;
                sum_1 += product;
            ...
            if row_2 < $rows {
                let left =
                    Bf16::from_bits(memory::volatile_load($a, row_2 * 1024 + inner)).to_f32();
                let product = left * right;
                sum_2 += product;
            ...
            if row_3 < $rows {
                let left =
                    Bf16::from_bits(memory::volatile_load($a, row_3 * 1024 + inner)).to_f32();
                let product = left * right;
                sum_3 += product;
...
#[kernel(typed, launch(required = [64, 1, 1], max = [64, 1, 1], max_grid = [18992, 1, 1]), control_flow(loop_bounds(1024)))]
#[allow(clippy::too_many_arguments, clippy::manual_div_ceil)]
pub fn ferric_qwen3_draft_batch32_head_bf16_f32_v10(
    a: &[u16],
    weights: &[u16],
    ...
    if rows == 0 || rows > 32 || n != 151936 || k != 1024 || world_size != 1 || projection != 6 {
        fe2o3_device::trap();
    }
    ...
    if a.len() < rows * 1024
        || a.len() > 32 * 1024
        || weights.len() != 151936 * 1024
        || output.len() < rows * 151936
        || output.len() > 32 * 151936
        || thread::launch_extent_1d() != ((rows + 15) / 16) * 9496 * 64
    {
        fe2o3_device::trap();
    }
    ...
    let tile_row = tile_index / 9496;
    let tile_column = tile_index % 9496;
    let lane = raw % 64;
    if tile_row < 2 {
    } else {
        fe2o3_device::trap();
    ...
    let (sum_0, sum_1, sum_2, sum_3) = {
        ...
        while inner < 1024 {
            let right =
                Bf16::from_bits(memory::volatile_load(weights, column * 1024 + inner)).to_f32();
            if row_0 < rows {
                let left = Bf16::from_bits(memory::volatile_load(a, row_0 * 1024 + inner)).to_f32();
                let product = left * right;
                sum_0 += product;
                if !product.is_finite() || !sum_0.is_finite() {
                    fe2o3_device::trap();
            ...
            if row_1 < rows {
                let left = Bf16::from_bits(memory::volatile_load(a, row_1 * 1024 + inner)).to_f32();
                let product = left * right;
                sum_1 += product;
                if !product.is_finite() || !sum_1.is_finite() {
                    fe2o3_device::trap();
            ...
            if row_2 < rows {
                let left = Bf16::from_bits(memory::volatile_load(a, row_2 * 1024 + inner)).to_f32();
                let product = left * right;
                sum_2 += product;
                if !product.is_finite() || !sum_2.is_finite() {
                    fe2o3_device::trap();
            ...
            if row_3 < rows {
                let left = Bf16::from_bits(memory::volatile_load(a, row_3 * 1024 + inner)).to_f32();
                let product = left * right;
                sum_3 += product;
                if !product.is_finite() || !sum_3.is_finite() {
                    fe2o3_device::trap();
...
#[kernel(typed, launch(required = [64, 1, 1], max = [64, 1, 1], max_grid = [18992, 1, 1]), control_flow(loop_bounds(64)))]
#[allow(clippy::too_many_arguments, clippy::manual_div_ceil)]
pub fn ferric_qwen3_draft_batch32_mfma_head_f32_v10(
    a: &[u16],
    weights_kn: &[u16],
    mut output: WriteOnlyDisjointSlice<f32, Tiled2D<Index1D, 64, 16, 16, 4>>,
    rows: u32,
    ...
    if a.len() < rows * 1024 || a.len() > 32 * 1024 || weights_kn.len() != 151936 * 1024 {
        fe2o3_device::trap();
    }
    let invocation = thread::index_1d();
    let raw = invocation.get();
    ...
    // Retained guards make these narrowings lossless and expose bounded offsets.
    let tile_row = tile_row as u8 as usize;
    if tile_column < 9496 {
    } else {
        fe2o3_device::trap();
    ...
    let Ok(right) = Bf16MfmaBMatrix::row_major(weights_kn, 0, 1024, 151936, 151936) else {
        fe2o3_device::trap();
    };
    ...
    while step < 64 {
        let left_fragment = left.load_m16k16(&lane, tile_row * 16, step * 16);
    ...
    if output.len() < rows * 151936 || output.len() > 32 * 151936 {
        fe2o3_device::trap();
    }
    if tile_column < 9496 {
    } else {
    ...
    if thread::grid_dim_x() as usize != ((rows + 15) / 16) * 9496 || thread::block_dim_x() != 64 {
        fe2o3_device::trap();
    }
    let Some(tile) = invocation.checked_tiled_2d::<64, 16, 16, 4>() else {
        fe2o3_device::trap();
...
mod tests {
    ...
    fn serial_body_matches_independent_real_width_reference_and_masks_inactive_rows() {
        for rows in [
            1_usize, 2, 3, 4, 5, 6, 15, 16, 17, 18, 19, 20, 29, 30, 31, 32,
        ] {
            let a: std::vec::Vec<_> = (0..rows * 1024)
                .map(|i| Bf16::from_f32(((i * 17 % 37) as f32 - 18.0) / 32.0).to_bits())
                .collect();
            let weights: std::vec::Vec<_> = (0..3 * 1024)
                .map(|i| Bf16::from_f32(((i * 11 % 43) as f32 - 21.0) / 64.0).to_bits())
                .collect();
            for column in 0..3 {
                for base in [0, 4, 8, 12, 16, 20, 24, 28] {
                    ...
                    for (offset, actual) in [a0, a1, a2, a3].into_iter().enumerate() {
                        ...
                        if row < rows {
                            for inner in 0..1024 {
                                reference +=
                                    f64::from(Bf16::from_bits(a[row * 1024 + inner]).to_f32())
                                        * f64::from(
                                            Bf16::from_bits(weights[column * 1024 + inner])
                                                .to_f32(),
                                        );
                            }
                        }
    ...
    fn serial_nonfinite_and_overflow_trap() {
        for (left, right) in [(f32::NAN, 1.0), (1.0, f32::INFINITY), (f32::MAX, 2.0)] {
            let a = std::vec![Bf16::from_f32(left).to_bits(); 1024];
            let weights = std::vec![Bf16::from_f32(right).to_bits(); 1024];
