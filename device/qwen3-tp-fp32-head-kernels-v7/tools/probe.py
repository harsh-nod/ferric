#!/usr/bin/env python3
"""Nine bounded full-vocabulary FP32-head fixtures. No model or rate qualification."""
import argparse
import array
import hashlib
import json
...
N, K, CAPACITY = 151936, 4096, 16
POSITIONS = (0, 1, 15, 16, 4095)
ROOTS = ("ferric_qwen3_tp_head_bf16_f32_v7", "ferric_qwen3_tp_mfma_head_f32_v7", "ferric_qwen3_tp_argmax_f32_v7")
...
def head_case(core, rows, mfma):
    ...
    return {"name": f"{'mfma' if mfma else 'scalar'}_head_rows{rows}", "symbol": ROOTS[int(mfma)],
            "groups": N // 16, "scalars": [rows, N, K, 1, 6],
...
def self_test(core):
    ...
    print("PASS: nine native cases specified; full-vocabulary reference winners, 128x4096 full transpose; no GPU")
...
def main():
    ...
    try:
        ...
        report = {"schema": "FerricTpFp32HeadProbeV7", "authority": "none", "benchmark": False,
