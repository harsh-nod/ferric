use ferric_qwen3_tp_fp32_head_kernels_device_v7::{ROOTS_V7, compiler_expectation_roster_v7};
...
fn closed_three_root_roster_and_exact_argument_widths() {
    let roster = compiler_expectation_roster_v7();
    ...
    for (name, expected_bytes) in [(ROOTS_V7[0], 68_u32), (ROOTS_V7[1], 68), (ROOTS_V7[2], 36)] {
...
fn every_active_output_has_one_owner_and_all_tail_rows_have_none() {
    for rows in 1..=16_usize {
        let mut owners = vec![0_u8; 16 * 151936];
        for group in 0..9496 {
            for lane in 0..64 {
...
fn source_preserves_input_guards_fp32_stores_and_zero_filled_mfma_rows() {
    ...
    let mfma = PROJECTION
        .split("pub fn ferric_qwen3_tp_mfma_head_f32_v7")
        .nth(1)
        .unwrap()
        .split("#[cfg(test)]")
        .next()
    ...
    let argmax = LOGITS
        .split("pub fn ferric_qwen3_tp_argmax_f32_v7")
        .nth(1)
        .unwrap()
        .split("#[cfg(test)]")
        .next()
