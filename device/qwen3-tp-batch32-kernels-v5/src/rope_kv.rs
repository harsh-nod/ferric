...
#[cfg(test)]
macro_rules! batch_paged_slot_v5 {
...
#[cfg(test)]
macro_rules! batch_distinct_slots_v5 {
    ($positions:expr, $tables:expr, $rows:expr, $stride:expr, $pages:expr) => {{
        let mut row = 0_usize;
        while row < $rows {
            let slot = batch_paged_slot_v5!($positions, $tables, row, $stride, $pages);
            let mut other = 0_usize;
            while other < $rows {
                if other < row {
                    let previous =
                        batch_paged_slot_v5!($positions, $tables, other, $stride, $pages);
                    if slot == previous {
                        fe2o3_device::trap();
                    }
                }
...
#[kernel(typed, launch(required = [64, 1, 1], max = [64, 1, 1], max_grid = [32, 1, 1]), control_flow(loop_bounds(32, 8)))]
#[allow(clippy::too_many_arguments)]
pub fn ferric_qwen3_tp_batch32_rope_v5(
    query: &[u16],
    key: &[u16],
    cos: &[f32],
    sin: &[f32],
...
#[allow(clippy::too_many_arguments)]
pub fn ferric_qwen3_tp_batch32_paged_kv_append_v5(
    key: &[u16],
    value: &[u16],
    positions: &[u32],
    page_table: &[u32],
    ...
    {
        // BEGIN batch_distinct_slots_v5
        let mut row = 0_usize;
        while row < rows {
            let slot = {
                // BEGIN batch_paged_slot_v5
                let row = row;
