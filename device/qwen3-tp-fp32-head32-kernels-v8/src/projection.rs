...
#[kernel(typed, launch(required = [64, 1, 1], max = [64, 1, 1], max_grid = [18992, 1, 1]), control_flow(loop_bounds(4096)))]
#[allow(clippy::too_many_arguments, clippy::manual_div_ceil)]
pub fn ferric_qwen3_tp_batch32_head_bf16_f32_v8(
    a: &[u16],
    weights: &[u16],
    ...
    if rows == 0 || rows > 32 || n != 151936 || k != 4096 || world_size != 1 || projection != 6 {
        fe2o3_device::trap();
    }
...
#[allow(clippy::too_many_arguments, clippy::manual_div_ceil)]
pub fn ferric_qwen3_tp_batch32_mfma_head_f32_v8(
    a: &[u16],
    weights_kn: &[u16],
    mut output: WriteOnlyDisjointSlice<f32, Tiled2D<Index1D, 64, 16, 16, 4>>,
    rows: u32,
    ...
    if a.len() < rows * 4096 || a.len() > 32 * 4096 || weights_kn.len() != 151936 * 4096 {
        fe2o3_device::trap();
    }
    let invocation = thread::index_1d();
    let raw = invocation.get();
