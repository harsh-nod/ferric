use ferric_qwen3_tp_large_kv_kernels_device_v9::{ROOTS_V9, compiler_expectation_roster_v9, contract};
...
const OLD_ATTENTION: &str = include_str!("../../qwen3-tp-batch32-kernels-v5/src/baseline_attention.rs");
...
fn closed_two_root_roster_preserves_explicit_abis_and_launch_geometry() {
    ...
    assert!(roster.windows(2).all(|pair| pair[0].kernel_binding_id() < pair[1].kernel_binding_id()));
    let mut names: Vec<_> = roster.iter().map(|entry| {
    ...
    for (source, name, expected_bytes) in [
        ...
        let functions: Vec<_> = parsed.items.iter().filter_map(|item| match item {
        ...
        let bytes: u32 = function.sig.inputs.iter().map(|argument| {
            let FnArg::Typed(argument) = argument else { panic!("receiver") };
...
fn exact_v5_root_and_helper_copies_change_only_declared_scope() {
    let macros = OLD_APPEND.split("#[cfg(test)]\nmacro_rules! batch_rope_pair_v5").next().unwrap();
    let macros = &macros[macros.find("#[cfg(test)]").unwrap()..];
    let body = &OLD_APPEND[OLD_APPEND.find("/// One grid leader").unwrap()..];
    let old = format!("use fe2o3_device::{{GridExclusive, WriteOnlyDisjointSlice, kernel, memory, thread}};\n\n{macros}{body}");
    for (old, actual) in [(old.as_str(), APPEND), (OLD_ATTENTION, ATTENTION)] {
        let expected = old.replace("_v5", "_v9")
            ...
            .replace("!(world_size == 1 || world_size == 2 || world_size == 8)", "world_size != 1")
...
fn physical_and_logical_limits_are_independent_and_bounded() {
        ...
        for world in [0, 1, 2, 8, u32::MAX] {
            for stride in [0, 1, 512, 513, u32::MAX] {
                for pages in [0, 1, 511, 512, 513, 8192, 16384, 16385, u32::MAX] {
                    assert_eq!(contract::supported(rows, world, stride, pages),
                        (1..=32).contains(&rows) && world == 1 && (1..=512).contains(&stride) && (1..=16384).contains(&pages));
                }
            }
        }
    }
    ...
    assert_eq!(u64::from(contract::MAX_CACHE_ELEMENTS) * 2 * 2 * 36, 38654705664);
