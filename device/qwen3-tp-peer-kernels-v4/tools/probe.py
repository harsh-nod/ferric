...
REDUCE = "ferric_qwen3_tp_peer_ordered_residual_bf16_v4"
...
def cases(core):
    for rows in (1, 3, 16):
        ...
        data = b"".join(struct.pack("<H", index if rows == 16 else patterns[index % 8])
                        for index in range(elements))
...
def self_test(core):
    ...
    core.require(len(fixtures) == 13 and {case["symbol"] for case in fixtures} == {REDUCE, COPY}, "closed fixture roster")
    for case in fixtures:
        rows = case["scalars"][0]
        core.require(case["groups"] == rows * 64 and 1 <= rows <= 16, "launch bounds")
...
def main():
    ...
    if args.self_test:
        ...
        print("PASS: 13 bounded arithmetic fixtures; no GPU worker launched")
        return
    ...
    try:
        ...
        report = {"schema": "FerricTpPeerArithmeticProbeV4", "authority": "none",
        ...
        with (args.output / "result.json").open("x", encoding="utf-8") as output:
            ...
            output.write("\n")
        print("PASS: 13 GPU arithmetic probes; no peer-mapping or benchmark claim")
