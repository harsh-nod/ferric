...
#[cfg(test)]
macro_rules! batch_paged_slot_v2 {
...
#[cfg(test)]
macro_rules! batch_distinct_slots_v2 {
    ($positions:expr, $tables:expr, $rows:expr, $stride:expr, $pages:expr) => {{
        let mut row = 0_usize;
        while row < $rows {
            let slot = batch_paged_slot_v2!($positions, $tables, row, $stride, $pages);
            let mut other = 0_usize;
            while other < $rows {
                if other < row {
                    let previous =
                        batch_paged_slot_v2!($positions, $tables, other, $stride, $pages);
                    if slot == previous {
                        fe2o3_device::trap();
                    }
                }
...
#[cfg(test)]
macro_rules! batch_rope_pair_v2 {
...
#[kernel(typed, launch(required = [64, 1, 1], max = [64, 1, 1], max_grid = [16, 1, 1]), control_flow(loop_bounds(32, 8)))]
#[allow(clippy::too_many_arguments)]
pub fn ferric_qwen3_tp_batch_rope_v2(
    query: &[u16],
    key: &[u16],
    cos: &[f32],
    sin: &[f32],
    ...
    if rows == 0 || rows > 16 || !(world_size == 1 || world_size == 2 || world_size == 8) {
        fe2o3_device::trap();
    }
    ...
    if query.len() < rows * query_columns
        || query.len() > 16 * query_columns
        || key.len() < rows * key_columns
        || key.len() > 16 * key_columns
        || rotated_query.len() < rows * query_columns
        || rotated_query.len() > 16 * query_columns
        || rotated_key.len() < rows * key_columns
        || rotated_key.len() > 16 * key_columns
        || cos.len() < rows * 64
        || cos.len() > 1024
        || sin.len() < rows * 64
        || sin.len() > 1024
...
#[kernel(typed, launch(required = [64, 1, 1], max = [64, 1, 1], max_grid = [1, 1, 1]), control_flow(loop_bounds(16, 16, 16, 1024)))]
#[allow(clippy::too_many_arguments)]
pub fn ferric_qwen3_tp_batch_paged_kv_append_v2(
    key: &[u16],
    value: &[u16],
    positions: &[u32],
    page_table: &[u32],
