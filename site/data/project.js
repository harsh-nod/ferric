window.FERRIC_PROJECT = Object.freeze({
  ...
  residentCheckpoint: Object.freeze({
    ...
    overview: "Private authenticated S1 K4/K8/K16 draft KV catch-up is integrated and host-tested. One generated 128-token prefill case now has a complete 151,936-logit comparison against an independent BF16/SDPA reference: the chosen token matches, but the rows are not byte-identical and no tolerance is accepted. Separately, the latest 4f6 compiler emits and replays the 12-root gfx942 image; that image is not GPU-tested. All 33 M1 gates remain open; no new serving or performance result is claimed.",
    ...
    remaining: "Next: review numerical acceptance criteria, complete the remaining R29 cases and exercise the integrated resident catch-up path. The selected logit comparison accepts no tolerance; neither target-only observation qualifies resident speculative serving. Latest combined host validation is separate from the retained bb2/ccfd cohorts below. No native K8/K16 qualification, new TTFT/TPOT/throughput measurement or competitiveness claim is admitted here. All 33 M1 gates remain open. Historical measurements below are unchanged.",
  }),
