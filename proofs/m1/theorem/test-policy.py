...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ferric-m1-theorem-registry.") as scratch:
        ...
        if len(active.read_text(encoding="ascii").splitlines()) != 23:
            fail("baseline theorem registry did not select exactly twenty-three rows")
            ...
            (
                ...
                lambda fixture: mutate_registry(
                    ...
                    lambda lines: lines.append(
                        lines[registry_row(lines, "batching-publish-once")].replace(
                            "batching-publish-once", "unknown-row"
                        )
                    ),
                ),
            ),
