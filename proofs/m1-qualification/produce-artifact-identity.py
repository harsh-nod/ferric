#!/usr/bin/env python3
"""Produce one source-bound, identity-only M1 artifact report and payload."""
...
IDENTITY_REPORT_FORMAT = "FERRIC-M1-ARTIFACT-IDENTITY-V1"
IDENTITY_ARTIFACT_KIND = "M1ImmutablePayload"
IDENTITY_AUTHORITY = "identity-and-structure-only"
IDENTITY_NONCLAIM = (
...
ARTIFACT_IDENTITY_ROSTER_SHA256 = (
    "036a350d44c964bd96c44328087d541db7116452093ed9067987fa8497e57258"
)
ARTIFACT_IDENTITY_TSV_SHA256 = (
    "b53369f60d5f767b67ae2a2a3044e656f389943cab7017bd8931a87f8c03d7f8"
)
...
def validate_requirements(requirements: JsonObject) -> None:
    ...
    if any(
        ...
        fail("artifact-identity production requires every M1 obligation to remain Open")
    for key in (
        "m0_contracts_commit",
        "m1_upstream_base_commit",
        "m1_upstream_base_tree",
...
def rederive_candidate_plan(
    ...
    with tempfile.TemporaryDirectory(
        prefix="ferric-m1-artifact-planner-replay-"
    ) as raw:
...
def validate_plan(
    ...
    if (
        ...
        or queue["counts"]
        != {
            "available_producer_items": 133,
            "missing_items": 358,
            "missing_producer_items": 225,
        }
    ):
    ...
    if any(
        ...
        fail("artifact-identity production refuses a plan containing a closure output")
    if replay:
...
def select_artifact_identity_binding(
    ...
    if not isinstance(binding_id, str) or not binding_id.startswith("binding."):
        fail(f"unknown M1 artifact-identity binding: {binding_id}")
    slots = [
        slot
        for slot in plan["binding_slots"]
        if slot.get("binding", {}).get("evidence_kind") == "artifact-identity"
    ]
    if len(slots) != 74:
        fail("M1 artifact-identity binding roster is incomplete")
    ...
    for slot in slots:
        ...
        if (
            ...
            fail(f"M1 artifact-identity producer contract drifted: {binding['id']}")
        for value, counts in (
    ...
    if (
        class_counts != {"Assurance": 32, "Roadmap": 42}
        or source_counts != {"source.fe2o3": 9, "source.ferric": 65}
        or profile_counts
        != {
            "admission": 6,
            "authentication": 8,
    ...
    if len(resolution_matches) != 1:
        fail("selected M1 artifact-identity path resolution drifted")
...
def requirement_spec(
    ...
    if obligation_class == "Roadmap":
        ...
        if len(matches) != 1:
            fail(
                "selected artifact-identity binding names an unknown roadmap obligation"
            )
    ...
    if obligation_class == "Assurance":
        ...
        if len(matches) != 1:
            fail(
                "selected artifact-identity binding names an unknown assurance property"
            )
        return matches[0], [obligation_id]
    fail("selected artifact-identity obligation class drifted")
...
def artifact_identity_report(
    ...
    if (
        ...
        fail("selected artifact-identity obligation or path drifted")
    artifact_id = binding["artifact_id"]
    return {
        "artifact_kind": IDENTITY_ARTIFACT_KIND,
        "artifact_profile_id": binding["profile_id"],
        "artifact_relative_path": f"identified-artifacts/{artifact_id}.bin",
        ...
        "format": IDENTITY_REPORT_FORMAT,
        "nonclaim": IDENTITY_NONCLAIM,
        "obligation_class": binding["obligation_class"],
        "obligation_id": binding["obligation_id"],
        "obligation_state": "Open",
...
def publish_identity(
    ...
    payload_fd = ensure_output_directory(
        plan_fd, "identified-artifacts", "M1 identified-artifact directory"
    )
    payload_file_fd = -1
    report_fd = -1
    ...
    try:
        ...
        revalidate_child_directory(
            ...
            payload_fd,
            "M1 identified-artifact directory",
        )
        payload_name = f"{artifact_id}.bin"
        report_name = f"{artifact_id}.artifact-identity.json"
        if entry_exists_at(payload_fd, payload_name) or entry_exists_at(
            artifact_fd, report_name
        ):
            fail("artifact-identity publication refuses a preexisting output")
        payload_file_fd = create_new_file_at(
            payload_fd, payload_name, payload, "M1 identified artifact payload"
        )
        ...
        report_fd = create_new_file_at(
            artifact_fd, report_name, report, "M1 artifact-identity report"
        )
        ...
        if report_fd >= 0:
            rollback_exact_file(
                artifact_fd,
                f"{artifact_id}.artifact-identity.json",
                report_fd,
                "M1 artifact-identity report",
            )
        fail(f"cannot durably publish M1 artifact identity: {error}")
    except BaseException:
        if report_fd >= 0:
            rollback_exact_file(
                artifact_fd,
...
def produce(
    ...
    try:
        ...
        slot, resolution = select_artifact_identity_binding(plan, queue, binding_id)
        ...
        if repeated[5] != plan_raw or repeated[6] != queue_raw:
            fail("M1 plan or work queue changed during artifact-identity production")
        plan_files = [
        ...
        repeated_slot, repeated_resolution = select_artifact_identity_binding(
        ...
        repeated_payload, source_custody = open_source_file(
        ...
        if (
            ...
            fail("M1 artifact-identity inputs changed during production")
        ...
        def revalidate_completion_inputs() -> None:
            ...
            if any(
                ...
                fail("artifact-identity producer created a forbidden closure output")
            revalidate_directory_path(plan_root, plan_fd, "M1 evidence plan directory")
        ...
        os.close(ferric_fd)
    print(
        f"PASS: produced M1 artifact identity binding={binding_id} "
        f"payload_sha256={digest_bytes(payload)} report_sha256={digest_bytes(report_bytes)}"
    )
