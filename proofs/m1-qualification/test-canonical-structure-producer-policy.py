#!/usr/bin/env python3
"""Exercise all 14 planner-bound M1 canonical-structure producers."""
...
def commit_fixture(repository: Path) -> None:
    ...
    command(
        [
            ...
            str(repository),
            "-c",
            "user.name=M1 Canonical Structure Policy",
            "-c",
            "user.email=m1-canonical-structure@example.invalid",
            "commit",
            "--quiet",
            "-m",
            "add M1 canonical-structure producer",
        ],
        "commit fixture",
    )
...
def expect_failure(
    ...
    if result.returncode == 0 or expected not in result.stdout:
        fail(
            f"canonical producer accepted hostile input; expected {expected!r}:\n"
            f"{result.stdout}"
        )
...
def run_planner(planner: Path, ferric: Path, fe2o3: Path, output: Path) -> None:
    ...
    if result.returncode != 0:
        fail(f"planner rejected canonical producer fixture:\n{result.stdout}")
...
def canonical_slots(plan: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        slot
...
def validate_all_reports(
    ...
    observed = []
    for slot in canonical_slots(plan):
        binding = slot["binding"]
        artifact = slot["expected_artifact"]
        artifact_id = artifact["id"]
        report_path = plan_root / artifact["path"]
        payload_path = plan_root / "canonical-payloads" / f"{artifact_id}.json"
        source_path = (
        ...
        }[resolutions[binding["path_id"]]["availability"]]
        expected_records = [
            {
            ...
            {"name": "source_is_regular", "type": "boolean", "value": True},
            {
                ...
                "type": "text",
                "value": resolutions[binding["path_id"]]["path"],
            },
        ]
        if payload_value["records"] != expected_records:
            fail(f"producer canonical payload projection drifted: {binding['id']}")
    ...
    if (
        ...
        fail("validated canonical-structure roster drifted")
...
def load_producer(path: Path) -> ModuleType:
    ...
    if specification is None or specification.loader is None:
        fail("cannot load canonical producer race policy")
...
def publication_races(root: Path, baseline: Path, producer: Path) -> int:
    ...
    for race in ("plan", "artifact", "payload"):
        ...
        def intercept(
            ...
            if not replaced:
                ...
                if race == "plan":
                    ...
                    current = plan / "canonical-payloads"
                    current.rename(plan / "canonical-payloads-held")
                    current.mkdir(mode=0o700)
        ...
        expected = {
            ...
            "payload": "canonical-payload directory was replaced",
        }[race]
        try:
            expect_direct_failure(
                lambda: module.publish_canonical_structure(
                    plan,
                    plan_fd,
                    artifact_fd,
            ...
            os.close(plan_fd)
        report_name = "artifact.binding.00000.canonical-structure.json"
        ...
        if any(path.exists() for path in candidates):
            fail(
                f"{race} replacement left a false canonical-structure completion marker"
            )
        cases += 1
    return cases
...
def completion_marker_failures(root: Path, baseline: Path, producer: Path) -> int:
    ...
    for phase, failure_call in (("pre-report", 1), ("post-report", 2)):
        ...
        report = plan / "artifacts" / f"{artifact_id}.canonical-structure.json"
        payload = plan / "canonical-payloads" / f"{artifact_id}.json"
        try:
            expect_direct_failure(
                lambda: module.publish_canonical_structure(
                    plan,
    ...
    for target_kind, artifact_id in (
        ...
        def mutate_after_readback() -> None:
            ...
            if target_kind == "payload":
                target = plan / "canonical-payloads" / f"{artifact_id}.json"
            else:
                target = plan / "artifacts" / f"{artifact_id}.canonical-structure.json"
        ...
        description = (
            ...
            if target_kind == "payload"
            else "M1 canonical-structure report"
        )
        try:
            expect_direct_failure(
                lambda: module.publish_canonical_structure(
...
def repository_commit_transition_race(
    ...
    command(
        [
            ...
            str(repository),
            "-c",
            "user.name=M1 Canonical Structure Race",
            "-c",
            "user.email=m1-canonical-structure-race@example.invalid",
            "commit",
            "--quiet",
            "-m",
            "first",
    ...
    artifact_id = "artifact.binding.00001"
    report = plan / "artifacts" / f"{artifact_id}.canonical-structure.json"
    calls = 0
    ...
    try:
        ...
        if report.exists():
            fail("clean commit transition left a false canonical completion marker")
    finally:
        os.close(artifact_fd)
        os.close(plan_fd)
...
def source_closure_races(root: Path, baseline: Path, producer: Path) -> int:
    ...
    for race, artifact_id in (
        ("replacement", "artifact.binding.00003"),
        ("in-place", "artifact.binding.00006"),
    ):
...
def hostile_inputs(
    ...
    (output_permissions / "canonical-payloads").mkdir(mode=0o755)
    (output_permissions / "canonical-payloads").chmod(0o755)
    expect_failure(
    ...
    (output_symlink / "canonical-payloads").symlink_to(target_dir)
    expect_failure(
        producer,
        ferric,
        fe2o3,
    ...
    selected = next(
        slot
        for slot in canonical_slots(read_json(baseline / "plan.json"))
    ...
    payload_dir = partial / "canonical-payloads"
    payload_dir.mkdir(mode=0o700)
    (payload_dir / f"{artifact_id}.json").write_bytes(b"hostile\n")
...
def main() -> None:
    ...
    producer_source = (
        repo / "proofs/m1-qualification/produce-canonical-structure.py"
    ...
    if any(token in producer_source for token in forbidden_copy_paths):
        fail("canonical producer retains a stale TCB publication path")
    with tempfile.TemporaryDirectory(prefix="ferric-m1-artifact-producer-") as raw:
        ...
        producer = ferric / "proofs/m1-qualification/produce-canonical-structure.py"
        validator = ferric / "proofs/m1/evidence/validate-canonical-structure.py"
        ...
        slots = canonical_slots(plan)
        if len(slots) != 14:
            fail("planner did not expose exactly 14 canonical-structure bindings")
        ...
        if any(
            ...
            fail("canonical producer emitted a forbidden closure output")
    ...
    print(
        "PASS: M1 canonical-structure producer emitted and validated all 14 bindings "
