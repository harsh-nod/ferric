#!/usr/bin/env python3
"""Exercise all 15 planner-bound M1 external-contract producers."""
...
def commit_fixture(repository: Path) -> None:
    ...
    command(
        [
            ...
            str(repository),
            "-c",
            "user.name=M1 External Contract Policy",
            "-c",
            "user.email=m1-external-contract@example.invalid",
            "commit",
            "--quiet",
            "-m",
            "add M1 external-contract producer",
        ],
        "commit fixture",
    )
...
def expect_failure(
    ...
    if result.returncode == 0 or expected not in result.stdout:
        fail(
            f"external-contract producer accepted hostile input; expected {expected!r}:\n"
            f"{result.stdout}"
        )
...
def run_planner(planner: Path, ferric: Path, fe2o3: Path, output: Path) -> None:
    ...
    if result.returncode != 0:
        fail(f"planner rejected external-contract producer fixture:\n{result.stdout}")
...
def external_contract_slots(plan: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        slot
...
def validate_all_reports(
    ...
    observed = []
    for slot in external_contract_slots(plan):
    ...
    if (
        ...
        fail("validated external-contract roster drifted")
...
def load_producer(path: Path) -> ModuleType:
    ...
    specification = importlib.util.spec_from_file_location(
        "external_contract_producer", path
    )
    if specification is None or specification.loader is None:
        fail("cannot load external-contract producer race policy")
...
def publication_races(root: Path, baseline: Path, producer: Path) -> int:
    ...
    for race in ("plan", "artifact"):
        ...
        artifact_id = "artifact.binding.00064"
        report_name = f"{artifact_id}.external-contract.json"
        try:
            expect_direct_failure(
                lambda: module.publish_external_contract(
                    plan,
                    plan_fd,
                    artifact_fd,
                    artifact_id,
        ...
        if any(path.exists() for path in candidates):
            fail(f"{race} replacement left a false external-contract completion marker")
        cases += 1
    return cases
...
def completion_marker_failures(root: Path, baseline: Path, producer: Path) -> int:
    ...
    for phase, failure_call in (("pre-report", 1), ("post-report", 2)):
        ...
        artifact_id = "artifact.binding.00064"
        report = plan / "artifacts" / f"{artifact_id}.external-contract.json"
        try:
            expect_direct_failure(
                lambda: module.publish_external_contract(
                    plan,
            ...
            if report.exists():
                fail(
                    f"{phase} failure left a false external-contract completion marker"
                )
            module.publish_external_contract(
                plan,
                plan_fd,
                artifact_fd,
                artifact_id,
            ...
            if report.read_bytes() != b"{}\n":
                fail(
                    f"{phase} retry did not publish the exact external-contract report"
                )
        finally:
            os.close(artifact_fd)
            os.close(plan_fd)
...
def plan_file_races(root: Path, baseline: Path, producer: Path) -> int:
    ...
    for race, name, artifact_id in (
        ("replacement", "plan.json", "artifact.binding.00064"),
        ("in-place", "missing-work.json", "artifact.binding.00070"),
    ):
...
def published_byte_race(root: Path, baseline: Path, producer: Path) -> int:
    ...
    try:
        expect_direct_failure(
            ...
            "published M1 external-contract report bytes or binding changed",
        )
        if report.exists():
...
def repository_commit_transition_race(
    ...
    command(
        [
            ...
            "-c",
            "user.email=m1-external-contract-race@example.invalid",
            "commit",
            "--quiet",
            "-m",
            "first",
    ...
    artifact_id = "artifact.binding.00085"
    report = plan / "artifacts" / f"{artifact_id}.external-contract.json"
    calls = 0
    ...
    try:
        ...
        if report.exists():
            fail("clean commit transition left a false external-contract marker")
    finally:
        os.close(artifact_fd)
        os.close(plan_fd)
        os.close(repository_fd)
...
def source_closure_races(root: Path, baseline: Path, producer: Path) -> int:
    ...
    for race, artifact_id in (
        ("replacement", "artifact.binding.00094"),
        ("in-place", "artifact.binding.00103"),
    ):
...
def hostile_inputs(
    ...
    selected = next(
        slot
        for slot in external_contract_slots(read_json(baseline / "plan.json"))
    ...
    target_report = root / "hostile-external-contract-report"
    ...
    dirty = ferric / "external-contract-hostile-untracked"
...
def main() -> None:
    ...
    producer_source = (
        repo / "proofs/m1-qualification/produce-external-contract.py"
    ...
    if any(token in producer_source for token in forbidden_copy_paths):
        fail("external-contract producer retains a stale copied publication path")
    with tempfile.TemporaryDirectory(
        prefix="ferric-m1-external-contract-producer-"
    ) as raw:
        root = Path(raw)
        ferric = root / "ferric"
        ...
        producer = ferric / "proofs/m1-qualification/produce-external-contract.py"
        validator = ferric / "proofs/m1/evidence/validate-external-contract.py"
        ...
        slots = external_contract_slots(plan)
        if len(slots) != 15:
            fail("planner did not expose exactly 15 external-contract bindings")
        ...
        if any(
            ...
            fail("external-contract producer emitted a forbidden closure output")
        ...
        if command(
            ...
            fail("external-contract production dirtied an exact source repository")
    print(
        "PASS: M1 external-contract producer emitted and validated all 15 bindings "
