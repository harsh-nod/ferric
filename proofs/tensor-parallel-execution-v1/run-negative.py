#!/usr/bin/env python3
"""Actual-body sequence mutations, only in an exclusively owned remote stage."""
...
MUTATIONS = {
    "invalid_capacity": (
        "if capacity == 0 || capacity > 8192 || vocabulary == 0 {", "if false {"),
    "invalid_token": ("if token >= self.vocabulary {", "if false {"),
    "duplicate_begin": (
        "if self.running { return Err(TensorParallelSequenceErrorV1::Busy); }\n        if token",
        "if false { return Err(TensorParallelSequenceErrorV1::Busy); }\n        if token"),
    "exhausted_position": ("if self.position >= self.capacity {", "if false {"),
    "early_completion": ("if !self.running {", "if false {"),
    "poison_not_retained": ("{ self.poisoned = true; }", "{ self.poisoned = false; }"),
    "reset_reads_old_prefix": ("self.position = 0;", "self.position = self.position;"),
...
def main():
    ...
    source = repo / "crates/ferric-engine/src/tensor_parallel_execution.rs"
    ...
    command = [str(verus / "cargo-verus"), "build", "-p", "ferric-engine", "--locked",
               "--release", "--target-dir", str(target), "--fwd-verus-args-to", "roots",
               "-j", "1", "--lib", "--", "--no-cheating", "--output-json",
               "--verify-only-module", "tensor_parallel_execution"]
    results = []
    try:
        for name, (before, after) in MUTATIONS.items():
            ...
            clean = bounded(["cargo", "clean", "-p", "ferric-engine", "--release",
                             "--target-dir", str(target)], repo, env, 120)
            (output / f"{name}.clean.log").write_text(clean.stdout)
            clean.check_returncode()
            run = bounded(command, repo, env, 600)
            ...
            intended = any(marker in run.stdout for marker in [
                ...
                "possible arithmetic underflow/overflow", "loop invariant not satisfied",
            ])
            if run.returncode == 0 or not intended:
                raise RuntimeError(f"{name}: no intended Verus property rejection")
