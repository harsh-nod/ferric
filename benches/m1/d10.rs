...
fn main() -> ExitCode {
    ...
    if arguments
        .first()
        .is_some_and(|command| command == d10_collect::COMMAND)
    {
        return match d10_collect::collect_policy_observations(&arguments) {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => {
                eprintln!("FAIL: {error}");
                ExitCode::FAILURE
    ...
    if arguments
        .first()
        .is_some_and(|command| command == d10_observations::COMMAND)
    {
        return match d10_observations::validate_policy_observations(&arguments) {
            Ok(()) => ExitCode::SUCCESS,
            Err(error) => {
                eprintln!("FAIL: {error}");
                ExitCode::FAILURE
