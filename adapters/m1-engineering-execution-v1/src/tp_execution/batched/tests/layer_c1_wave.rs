...
fn configured(pool: &EngineeringTpPagedPoolV1) -> EngineeringTpBatchExecutionV2<Recording> {
    ...
    driver.projection = super::super::super::projection::ProjectionPolicy::synthetic_mfma_for_recording(
        original.id, transposed,
    );
...
fn select(driver: &mut EngineeringTpBatchExecutionV2<Recording>) -> TpResult<()> {
    driver.configure_ordered_c1_wave_layers_fp32_argmax_binding_v11(Fp32ArgmaxBindingV11::recording())
}
...
fn ordered_shape(transport: &Recording, published: bool) {
    ...
    let groups = events.iter().filter_map(|event| match event {
        Event::OrderedSubmit(_, count) => Some(*count),
        _ => None,
    }).collect::<Vec<_>>();
    assert_eq!(groups, [11, 6].repeat(36));
    assert_eq!(events.iter().filter(|event| matches!(event, Event::OrderedWait(_, _))).count(), 72);
    let mut cursor = 1;
    for count in groups {
        let group = &transport.commands[cursor..cursor + count];
        assert_eq!(group.last().unwrap().kernel, "ferric_qwen3_tp_batch32_residual_bf16_v5");
        assert!(group.iter().all(|command| !matches!(command.kernel, HEAD | ARGMAX_WAVE)));
        cursor += count;
    }
    if published {
    ...
    for rows in 1..=32 {
        ...
        for enabled in [false, true] {
            ...
            if enabled {
                ...
                driver.configure_ordered_wave_attention_fp32_argmax_binding_v11(Fp32ArgmaxBindingV11::recording()).unwrap();
            }
            assert_eq!(driver.layer_projection_mode(), if enabled { "c1-wave" } else { "mfma" });
            assert_eq!(driver.projection.mode, super::super::super::EngineeringTpProjectionModeV3::Mfma);
            assert_eq!(allocations(&driver), before);
            let original = driver.inner.ranks[0].global(Qwen3TensorKind::LanguageModelHead).id;
            ...
            let head = commands.iter().find(|command| command.kernel == HEAD).unwrap();
            ...
            for command in &mut commands {
                if matches!(command.kernel, MFMA | MFMA_PARTIAL | WAVE | WAVE_PARTIAL) {
                    ...
                    assert_eq!(buffer(command, 1).0, if wave { original } else { transposed });
                    assert_eq!(buffer(command, 2).3, if matches!(command.kernel, MFMA_PARTIAL | WAVE_PARTIAL) { 4 } else { 2 });
                    if wave {
                        waves += 1;
                        assert_eq!(command.grid_workgroups, rows * scalar(command, 4));
                        command.kernel = if command.kernel == WAVE { MFMA } else { MFMA_PARTIAL };
                        let EngineeringTpArgumentV1::Buffer { id, .. } = &mut command.arguments[1] else { panic!("weight") };
                        *id = transposed;
            ...
            recordings.push((commands, transport.reads.clone(), transport.writes.clone(), transport.write_payloads.clone(), transport.packet_preparations.clone(), before, output.choices.clone()));
            pool.commit_batch(&batch, output.completion).unwrap();
            pool.check_invariants().unwrap();
            pool.retire_sequence(batch.rows()[0].sequence(), false, 1).unwrap();
...
fn layer_c1_wave_rejection_is_atomic_for_every_existing_profile_boundary() {
    for mutation in 0..27 {
        ...
        match mutation {
            ...
            1 => { let mut other = Fp32ArgmaxBindingV11::recording(); other.hsaco[0] ^= 1; driver.admitted_argmax_v11 = Some(other); }
            ...
            23 => driver.inner.ranks.push(fixture(1, &wide_pool()).inner.ranks.pop().unwrap()),
            24 => driver.inner.transports.push(fixture(1, &wide_pool()).inner.transports.pop().unwrap()),
            25 => driver.c1_wave_layers = true,
            26 => driver.fp32_argmax_v11 = Some(Fp32ArgmaxBindingV11::recording()),
            _ => unreachable!(),
        }
        let before = (driver.c1_wave_layers, driver.fp32_argmax_v11, driver.inner.ordered_batches.clone());
        let io = || driver.inner.transports.iter().map(|transport| (transport.buffers.len(), transport.commands.clone(), transport.reads.clone(), transport.writes.clone(), transport.events.borrow().clone())).collect::<Vec<_>>();
        let io_before = io();
        assert!(select(&mut driver).is_err(), "mutation {mutation}");
        assert_eq!(before, (driver.c1_wave_layers, driver.fp32_argmax_v11, driver.inner.ordered_batches.clone()));
        let io_after = driver.inner.transports.iter().map(|transport| (transport.buffers.len(), transport.commands.clone(), transport.reads.clone(), transport.writes.clone(), transport.events.borrow().clone())).collect::<Vec<_>>();
...
fn layer_c1_wave_is_opt_in_and_terminal_without_broadening_old_selectors() {
    ...
    legacy.configure_ordered_wave_attention_fp32_argmax_binding_v11(Fp32ArgmaxBindingV11::recording()).unwrap();
    assert!(!legacy.c1_wave_layers);
    assert!(select(&mut legacy).is_err());
    for mode in [super::super::super::EngineeringTpProjectionModeV3::Wave, super::super::super::EngineeringTpProjectionModeV3::Auto] {
        let mut driver = configured(&wide_pool());
        driver.projection.mode = mode;
        assert!(driver.configure_ordered_wave_attention_fp32_argmax_binding_v11(Fp32ArgmaxBindingV11::recording()).is_err());
...
fn layer_c1_wave_invalid_selected_rows_have_no_transport_effects() {
    for (rows, selected) in [(1, vec![1]), (1, vec![0, 0]), (17, vec![17]), (17, vec![1, 0])] {
        let mut pool = wide_pool();
        let mut driver = configured(&pool);
        select(&mut driver).unwrap();
        let before = allocations(&driver);
...
fn layer_c1_wave_failures_poison_without_completion_and_quarantine_pool() {
    for failure in [Failure::WaveProjectionSubmit, Failure::WaveProjectionWait, Failure::OrderedSubmit, Failure::OrderedWait, Failure::AttentionSubmit, Failure::AttentionWait, Failure::ResidualSubmit, Failure::ResidualWait, Failure::ArgmaxSubmit, Failure::ArgmaxWait, Failure::BadChoice, Failure::PreparePackets] {
        let mut pool = wide_pool();
        let mut driver = configured(&pool);
        select(&mut driver).unwrap();
        ...
        assert!(driver.execute_selected(&batch, &[0]).is_err(), "{failure:?}");
        assert!(driver.poisoned);
        assert_eq!(driver.completed_batches(), 0);
        if matches!(failure, Failure::WaveProjectionSubmit | Failure::WaveProjectionWait | Failure::OrderedSubmit | Failure::OrderedWait) {
