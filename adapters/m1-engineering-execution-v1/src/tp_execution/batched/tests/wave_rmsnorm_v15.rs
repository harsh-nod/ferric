    ...
    for rows in [1, 16, 17, 32] {
        for selected in [
            ...
            for enabled in [false, true] {
                ...
                assert_eq!(
                    driver.rmsnorm_mode(),
                    if enabled { "wave-v15" } else { "baseline" }
                );
                ...
                for command in &mut commands {
                    if command.kernel == if enabled { V15 } else { RMSNORM }
                        && scalar(command, 6) == 4096
                    {
                ...
                if !selected.is_empty() {
                    expected_rows.push(u32::try_from(selected.len()).unwrap());
                }
                ...
                assert_eq!(
                    commands
                        .iter()
                        .filter(|c| c.kernel == "ferric_qwen3_tp_batch32_wave_paged_gqa_bf16_v5")
                        .count(),
                    36
                );
...
fn wave_rmsnorm_v15_terminal_rejection_is_atomic() {
    for mutation in 0..33 {
        ...
        let io = |driver: &EngineeringTpBatchExecutionV2<Recording>| {
            driver
                .inner
                .transports
                .iter()
...
fn wave_rmsnorm_v15_binding_requires_loaded_identity_before_allocation() {
    for mutation in 0..10 {
        ...
        let before = driver
            ...
            .map(|t| t.buffers.len())
            .collect::<Vec<_>>();
        let result = validate_wave_rmsnorm_binding_v15(
...
fn wave_rmsnorm_v15_invalid_batch_metadata_publishes_no_work() {
    for mutation in 0..6 {
        ...
        assert!(
            driver
                .execute_selected(&batch, &[if mutation == 5 { 17 } else { 0 }])
                .is_err()
        );
...
fn wave_rmsnorm_v15_failures_at_each_norm_site_never_publish_completion() {
    for ordinal in [0, 1, 72] {
        for failure in [
            Failure::RmsNormSubmit(ordinal),
            Failure::RmsNormWait(ordinal),
        ] {
            let mut pool = wide_pool();
            let mut driver = configured(&pool);
            select(&mut driver).unwrap();
            ...
            let error = driver
                ...
                .err()
                .expect("injected norm failure");
            assert!(
                error.contains("injected Wave RMSNorm"),
                "{failure:?}: {error}"
            );
...
fn wave_rmsnorm_v15_row_binding_is_exact_and_preserves_legacy_norms() {
    ...
    for rows in [1, 16, 17, 32] {
        ...
        command.kernel = V15;
        assert_eq!(
            bind_mode(false, 32, false, command.clone()).unwrap(),
            command
        );
        ...
        for index in 0..5 {
            for mutation in 0..5 {
                ...
                match mutation {
                    ...
                    3 => {
                        *access = if index >= 3 {
                            EngineeringTpBufferAccessV1::Read
                        } else {
                            EngineeringTpBufferAccessV1::Write
                        }
                    }
