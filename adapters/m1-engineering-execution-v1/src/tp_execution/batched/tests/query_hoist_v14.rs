...
fn select(driver: &mut EngineeringTpBatchExecutionV2<Recording>) -> TpResult<()> {
    driver.configure_ordered_c1_wave_query_hoist_binding_v14(
        Fp32ArgmaxBindingV11::recording(), QueryHoistBindingV14::recording(),
    )
}
    ...
    for rows in [1, 16, 17, 32] {
        for selected in [Vec::new(), vec![rows as usize - 1], (0..rows as usize).collect()] {
            ...
            for enabled in [false, true] {
                ...
                assert_eq!(driver.attention_mode(), if enabled { "query-hoist-v14" } else { "wave" });
                ...
                for command in &mut commands {
                    if command.kernel == if enabled { V14 } else { V5 } {
                        ...
                        assert_eq!((6..11).map(|i| scalar(command, i)).collect::<Vec<_>>(),
                            [rows, 1, 4, 4, rows]);
                        command.kernel = V5;
                    }
                }
                ...
                for event in &mut events {
                    if let Event::Submit(_, kernel) | Event::Wait(_, kernel) = event {
                        if *kernel == V14 { *kernel = V5; }
                    }
                }
                recordings.push((commands, transport.reads.clone(), transport.write_payloads.clone(),
                    transport.packet_preparations.clone(), events, output.choices.clone()));
                assert_eq!(allocations(&driver), before);
...
fn v14_preserves_mixed_position_causal_bound_and_decode_after_prefill() {
    ...
    for enabled in [false, true] {
        ...
        let fresh = pool.open_sequence(pool.scope(), &[99], 0).unwrap().sequence();
        let second = pool.reserve_batch(&[
            EngineeringTpPageRowV1 { sequence: fresh, token: 99, position: 0 },
            EngineeringTpPageRowV1 { sequence: continued, token: 100, position: 16 },
        ]).unwrap();
        ...
        let mut commands = transport.commands.clone();
        let attention = commands.iter().rev().find(|c| c.kernel == if enabled { V14 } else { V5 }).unwrap();
        assert_eq!(scalar(attention, 10), 17);
        for command in &mut commands {
            if command.kernel == V14 { command.kernel = V5; }
        }
...
fn v14_terminal_rejection_keeps_all_route_and_io_state_unchanged() {
    for mutation in 0..30 {
        ...
        match mutation {
            ...
            1 => { let mut b = QueryHoistBindingV14::recording(); b.hsaco[0] ^= 1; driver.admitted_query_hoist_v14 = Some(b); }
        ...
        let before = (driver.query_hoist_v14, driver.c1_wave_layers, driver.fp32_argmax_v11,
            driver.inner.ordered_batches.clone());
        let io_before = driver.inner.transports.iter().map(|t|
            (t.buffers.len(), t.commands.clone(), t.reads.clone(), t.writes.clone(), t.events.borrow().clone())
        ).collect::<Vec<_>>();
        assert!(select(&mut driver).is_err(), "mutation {mutation}");
        assert_eq!(before, (driver.query_hoist_v14, driver.c1_wave_layers, driver.fp32_argmax_v11,
            driver.inner.ordered_batches.clone()));
        assert_eq!(io_before, driver.inner.transports.iter().map(|t|
            (t.buffers.len(), t.commands.clone(), t.reads.clone(), t.writes.clone(), t.events.borrow().clone())
        ).collect::<Vec<_>>());
    }
}
...
fn v14_constructor_binding_checks_loaded_identity_before_allocation() {
    for mutation in 0..9 {
        let world = if mutation == 7 { 2 } else if mutation == 8 { 8 } else { 1 };
        ...
        assert!(validate_query_hoist_binding_v14(&mut driver.inner.transports,
            if mutation == 4 { 16 } else { 32 }, mutation == 5, binding).is_err());
    ...
    transport.buffers.clear();
    transport.query_hoist_v14_loaded = Some(QueryHoistBindingV14::recording().hsaco);
    assert!(transport.require_loaded_image(QueryHoistBindingV14::recording().hsaco,
        &crate::tp_artifact::ENGINEERING_TP_FP32_ARGMAX32_EXPORTS_V11).is_err());
    validate_query_hoist_binding_v14(&mut driver.inner.transports, 32, false, QueryHoistBindingV14::recording()).unwrap();
...
fn v14_invalid_rows_metadata_and_tokens_publish_no_transport_work() {
    for mutation in 0..6 {
        ...
        assert!(driver.execute_selected(&batch, &selected).is_err(), "mutation {mutation}");
...
fn v14_attention_failures_are_terminal_and_never_publish_completion() {
    for failure in [Failure::AttentionSubmit, Failure::AttentionWait, Failure::OrderedSubmit,
        Failure::OrderedWait, Failure::ArgmaxSubmit, Failure::ArgmaxWait, Failure::BadChoice] {
        let mut pool = wide_pool();
        let mut driver = configured(&pool);
        select(&mut driver).unwrap();
...
fn v14_direct_row_binding_rejects_wrong_geometry_and_leaves_v5_mapping_unchanged() {
    ...
    let original = driver.inner.transports[0].commands.iter().find(|c| c.kernel == V14).unwrap().clone();
    assert_eq!(bind_mode(false, 32, false, original.clone()).unwrap(), original);
    for mutation in 0..15 {
        let mut command = original.clone();
        match mutation {
            0 => { command.arguments.pop(); }
            ...
            12 => if let EngineeringTpArgumentV1::Buffer { element_bytes, .. } = &mut command.arguments[3] { *element_bytes = 2; },
            13 => if let EngineeringTpArgumentV1::Buffer { access, .. } = &mut command.arguments[5] { *access = EngineeringTpBufferAccessV1::Read; },
            14 => if let EngineeringTpArgumentV1::Buffer { access, .. } = &mut command.arguments[0] { *access = EngineeringTpBufferAccessV1::Write; },
            _ => unreachable!(),
        }
        assert!(bind_mode(false, 32, false, command).is_err(), "mutation {mutation}");
    }
    ...
    assert_eq!(bind_mode(false, 32, false, legacy.clone()).unwrap().kernel, V5);
