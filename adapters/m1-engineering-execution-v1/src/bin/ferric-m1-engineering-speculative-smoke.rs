...
const FAILURE_DIAGNOSTIC_BYTES: usize = 4096;
const FAILURE_DIAGNOSTIC_SUFFIX: &[u8] = b" [diagnostic truncated or formatting failed]\n";

struct BoundedFailureDiagnostic {
    bytes: [u8; FAILURE_DIAGNOSTIC_BYTES],
    len: usize,
}

impl std::fmt::Write for BoundedFailureDiagnostic {
    fn write_str(&mut self, text: &str) -> std::fmt::Result {
        let remaining = FAILURE_DIAGNOSTIC_BYTES - FAILURE_DIAGNOSTIC_SUFFIX.len() - self.len;
        let mut copied = text.len().min(remaining);
        while !text.is_char_boundary(copied) {
            copied -= 1;
        }
...
fn write_bounded_failure(
    sink: &mut impl std::io::Write,
    stage: &str,
    diagnostic: &dyn Debug,
) -> std::io::Result<()> {
    let mut buffer = BoundedFailureDiagnostic {
        bytes: [0; FAILURE_DIAGNOSTIC_BYTES],
        len: 0,
    };
    let formatted = std::fmt::Write::write_fmt(
        &mut buffer,
        format_args!("FAIL-STOP: {stage}: {diagnostic:?}"),
    );
    let suffix = if formatted.is_ok() {
        b"\n".as_slice()
    } else {
        FAILURE_DIAGNOSTIC_SUFFIX
    };
    buffer.bytes[buffer.len..buffer.len + suffix.len()].copy_from_slice(suffix);
    buffer.len += suffix.len();
    sink.write_all(&buffer.bytes[..buffer.len])
}

fn fail_stop(stage: &str, diagnostic: &dyn Debug) -> ! {
    let _ = write_bounded_failure(&mut std::io::stderr().lock(), stage, diagnostic);
    std::process::abort()
}
