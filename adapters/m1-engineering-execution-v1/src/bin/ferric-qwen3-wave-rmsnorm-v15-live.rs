//! Separate same-image `RMSNorm` live profile; no serving or benchmark grant.
...
mod wave_rmsnorm_v15_live_contract;
...
use wave_rmsnorm_v15_live_contract::{Options, RmsNormMode};

const LIVE_PROFILE: &str = "wave-rmsnorm-v15-live-v1";
...
fn profile_metadata(
    ...
    Ok(json!({
        ...
        "projection":"mfma", "attention":actual_attention, "argmax_mode":"wave-v11",
        ...
        "rmsnorm_mode":actual_rmsnorm, "rmsnorm_artifact_path":options.rmsnorm_artifact,
...
fn run_with_timing(options: &Options, timing: &mut TimingFile) -> Result<(), String> {
    ...
    let setup = json!({
        ...
        "rmsnorm_mode":rmsnorm_mode, "attention_mode":attention_mode,
        "benchmark_admitted":false, "serving_admitted":false,
...
fn main() -> std::process::ExitCode {
    match Options::parse(std::env::args().skip(1)).and_then(|options| run(&options)) {
        ...
        Err(error) => {
            eprintln!("Wave RMSNorm V15 live profile rejected: {error}");
...
mod live_tests {
    ...
    #[test]
    fn v15_live_metadata_uses_actual_modes_and_closed_runtime_bits() {
        for mode in [RmsNormMode::Baseline, RmsNormMode::WaveV15] {
            let options = wave_rmsnorm_v15_live_contract::fixture(mode);
            let profile = profile_metadata(&options, "c1-wave", "wave", mode.label()).unwrap();
            assert_eq!(
                profile,
                json!({
                    ...
                    "live_profile":LIVE_PROFILE, "layer_projection":"c1-wave",
                    "rmsnorm_mode":mode.label(), "rmsnorm_artifact_path":options.rmsnorm_artifact,
                    "benchmark_admitted":false, "serving_admitted":false,
                })
            );
            for (layer, attention, rmsnorm) in [
                ("mfma", "wave", mode.label()),
            ...
            assert!(profile_metadata(&mismatched, "c1-wave", "wave", mode.label()).is_err());
        }
    }

    #[test]
    fn invalid_direct_v15_live_options_create_no_timing_file_before_rejection() {
        let path = std::env::temp_dir().join(format!(
            "ferric-wave-rmsnorm-v15-live-reject-{}.json",
            std::process::id()
        ));
        assert!(!path.exists());
        for mode in [RmsNormMode::Baseline, RmsNormMode::WaveV15] {
            for mutation in 0..14 {
                let mut options = wave_rmsnorm_v15_live_contract::fixture(mode);
                ...
                match mutation {
                    ...
                    11 => options.live.context = 256,
                    12 => options.live.pages = 16,
                    13 => options.rmsnorm_artifact = PathBuf::new(),
                    _ => unreachable!(),
                }
