//! Separate non-authoritative live wave/v8/v11 profile; no protected serving grant.
...
use wave_argmax_live_contract::{CACHE_TTL, CHUNK, Options, ROWS, Submission};
...
fn profile_metadata(options: &Options) -> Result<Value, String> {
    options.validate()?;
    Ok(json!({
        ...
        "runtime_profiling":options.runtime.profile,
        "projection":"mfma", "attention":"wave", "argmax_mode":"wave-v11",
        "submission":options.submission.label(),
        "runtime_ordered_batches":options.runtime.ordered_batches,
    }))
}
...
fn run_with_timing(options: &Options, timing: &mut TimingFile) -> Result<(), String> {
    ...
    let performance_profile = profile_metadata(options)?;
    ...
    let setup = json!({
        ...
        "serving_qualified":false, "live_profile":"wave-argmax-live-v1",
        "model":"Qwen/Qwen3-8B", "dtype":"BF16", "target":"gfx950:xnack-",
        "tensor_parallel":1, "device_unique_ids":[options.device], "worker_pids":[worker_pid],
        "worker_sha256":options.worker_sha256, "running_worker_sha256":[options.worker_sha256],
        ...
        "head_precision":"fp32-v8", "argmax_mode":argmax_mode, "submission":options.submission.label(),
        "runtime_ordered_batches":options.runtime.ordered_batches, "performance_profile":performance_profile,
        "kernel_profile":"v5-mfma32", "kernel_row_capacity":ROWS, "batch_tokens":ROWS,
        "prefill_chunk":CHUNK, "page_tokens":16, "physical_pages":options.pages,
        "context_tokens":options.context, "cache_ttl_ticks":CACHE_TTL, "prefix_cache":false,
        ...
        "fp32_head_workspace_bytes":fp32_workspace_bytes, "max_batches":options.max_batches,
    ...
    let closed = json!({
        ...
        "live_profile":"wave-argmax-live-v1", "submission":options.submission.label(),
...
fn run(options: &Options) -> Result<(), String> {
    options.validate()?;
    let mut timing = TimingFile::create(options.host_timing.as_deref())?;
...
fn main() -> std::process::ExitCode {
    match Options::parse(std::env::args().skip(1)).and_then(|options| run(&options)) {
        ...
        Err(error) => {
            eprintln!("Wave argmax live profile rejected: {error}");
...
mod live_tests {
    ...
    #[test]
    fn live_profile_metadata_reports_the_actual_closed_runtime_bits() {
    ...
    fn invalid_direct_live_options_create_no_timing_file_before_rejection() {
        ...
        for submission in [Submission::Synchronous, Submission::Ordered] {
            for mutation in 0..7 {
                let mut options = wave_argmax_live_contract::fixture(submission);
                options.host_timing = Some(path.clone());
                match mutation {
