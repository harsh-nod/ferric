...
pub fn parse(arguments: impl Iterator<Item = String>) -> Result<(Options, PathBuf, CanaryProfile), String> {
        ...
        match flag.as_str() {
            "--attention-mode" => {
                ...
                profile = Some(match arguments.next().as_deref() {
                    ...
                    _ => return Err("attention mode must be resident-wave or query-hoist-v14".into()),
                });
            }
            "--query-hoist-artifact" => {
                ...
                let path = arguments.next().ok_or("missing query hoist artifact path")?;
                if path.is_empty() {
...
mod tests {
    ...
    fn arguments(mode: &str, outputs: &str) -> Vec<String> {
        ["--source", "source", "--target-artifact", "target", "--target-head-artifact", "head",
            "--argmax-artifact", "argmax", "--query-hoist-artifact", "v14", "--worker", "worker",
            ...
            "--host-timing-output", "timing", "--argmax-mode", "wave-v11", "--max-new-tokens", outputs,
            "--attention", "wave", "--submission", "ordered", "--layer-projection", "c1-wave",
            "--attention-mode", mode, "--allow-unauthenticated-machine-code", "--runtime-cache-admission",
    ...
    fn v14_canary_modes_share_image_workload_references_and_runtime() {
        ...
        for (outputs, packets) in [("8", 9_219), ("128", 83_139)] {
            ...
            for (mode, expected) in [("resident-wave", CanaryProfile::QueryHoistResidentWave),
                ...
                inputs.push((options.source, options.target_artifact, options.target_head_artifact,
                    options.argmax_artifact, artifact, options.worker, options.worker_sha256, options.device,
                    options.reference, options.prefix_reference, options.host_timing_output, options.outputs));
            }
    ...
    fn v14_canary_rejects_head_layer_submission_and_runtime_broadening() {
        for (flag, value) in [("--layer-projection", "mfma"), ("--submission", "synchronous"),
        ...
        for extra in [vec!["--runtime-sequences"], vec!["--runtime-profile"],
    ...
    fn v14_canary_preserves_option_shaped_path_values() {
        for flag in ["--source", "--target-artifact", "--target-head-artifact", "--argmax-artifact",
            "--query-hoist-artifact", "--worker", "--reference", "--prefix-reference", "--host-timing-output"] {
            for value in ["--attention-mode", "--query-hoist-artifact", "--layer-projection", "--runtime-operational"] {
                let mut args = arguments("query-hoist-v14", "8");
                let index = args.iter().position(|arg| arg == flag).unwrap();
                args[index + 1] = value.into();
