...
| Kernels | Wave attention plus v11 argmax passes finite fixtures and the earlier full128 ABBA cohort. Append `2716371` emits with 278 SGPR spills and is rejected. Corrected `d03ccd5` host validation passes fresh 19/19 profiles, but emission rejects a missing dominating overflow proof before producing an image. The nine-case append probe passes ten CPU tests. Visible-attention `4e287b1` source-fresh validation remains pending: R1 reused shared artifacts; R2 stopped before compilation at Cargo's cache-marker check. Corrected R3 has passed that cleanup step and is running. | Resolve the append range-proof obligation without weakening checks; complete visible R3. Both candidates still require admitted emission, native and performance validation. Exact known Clippy 101 debt remains explicit. V12 remains host/emission-only. |
| Runtime Optimization | Source candidate `2396a82` includes the TP1 residual in each existing ordered group: 11/6 packets instead of 10/5 plus singleton residuals. Packet order/counts and completion-before-state-update are preserved by the intended contract. Five new tests and stronger failure/equivalence checks are implemented; remote formatting passes. | Source-fresh adapter tests, strict Clippy, policy checks and release builds are pending. No new native correctness or performance claim. No fe2o3 change. |
