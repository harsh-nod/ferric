...
| Runtime Optimization | Tested source `2396a82` is integrated as `212ec85`, with exact code parity. All 31 fresh CPU gate steps pass: 780 adapter tests, 41 ignored, eight doctests, strict Clippy, 74 HTTP regressions and policy/release checks. Ordered groups now include residuals (11/6 packets); total packets and completion-before-state-update remain unchanged. | Native 8/128-output correctness checks, then a separate comparison. No new native or performance claim yet. No fe2o3 change. |
