...
/// Consumes a prepared owner into the Worker V3 binding stage.
#[must_use]
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BindQwen3LogitsWorkerV3ErrorV1 {
    /// The evidence's nested handoff differed from the prepared handoff.
    HandoffSubstitution,
}

impl fmt::Display for BindQwen3LogitsWorkerV3ErrorV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            formatter,
            "Qwen3 logits Worker V3 evidence binding failed: {self:?}"
        )
    }
}

impl std::error::Error for BindQwen3LogitsWorkerV3ErrorV1 {}
...
pub fn bind_qwen3_logits_worker_v3_v1(
    request: InertQwen3LogitsWorkerRequestV1,
    worker: InertProtectedFirstBuildWorkerV3EvidenceV1,
) -> Result<InertQwen3LogitsWorkerEvidenceV1, BindQwen3LogitsWorkerV3ErrorV1> {
    ...
    {
        return Err(BindQwen3LogitsWorkerV3ErrorV1::HandoffSubstitution);
    }
    Ok(InertQwen3LogitsWorkerEvidenceV1 { prepared, worker })
}
...
pub fn inspect_qwen3_logits_kernel_v1(
    ...
    let InertQwen3LogitsWorkerEvidenceV1 { prepared, worker } = evidence;
