...
pub(super) fn run(arguments: &[OsString]) -> CaptureResult<()> {
    let [source_root, prepacked_root, artifact_root, closure_path, gpu_unique_id, max_new_tokens, prompt] =
        arguments
    else {
        return Err("usage: ferric-m1-qualification-capture run-target-smoke MODEL-SOURCE PREPACKED-SNAPSHOT KERNEL-ARTIFACTS CLOSURE GPU-UNIQUE-ID MAX-NEW-TOKENS RAW-PROMPT".to_owned());
    };
    ...
    let source = super::SecureDirectory::open(Path::new(source_root), "model source root")?;
    let snapshot =
        super::SecureDirectory::open(Path::new(prepacked_root), "prepacked snapshot root")?;
    let model = load_model_inputs(&source, &snapshot)?;
    let tokenizer = authenticate_qwen3_tokenizer(
        Qwen3ModelRole::Target8B,
        Cursor::new(&model.target_tokenizer),
    )
...
mod tests {
    ...
    fn command_reports_the_exact_positional_usage() {
        ...
        assert!(error.contains("run-target-smoke MODEL-SOURCE PREPACKED-SNAPSHOT"));
