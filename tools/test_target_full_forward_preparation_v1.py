"""Synthetic preparation-fence worker policy fixtures, not GPU or reference evidence."""
...
import target_full_forward_preparation_v1 as check
from test_target_batch_decode_v1 import fixture as base_fixture


def fixture(variant="per-dispatch-checks"):
    ...
    plan.update(schema=check.EXPECTATION_SCHEMA, variant=variant,
                ...
                worker_source_manifest_sha256=check.WORKER_SOURCE_MANIFEST_SHA256,
                controller_sha256=check.CONTROLLER_SHA256, worker_sha256=check.WORKERS[variant],
    ...
    records[0]["running_worker_sha256"] = [check.WORKERS[variant]]
    for key in check.EXTRA_SETUP:
        records[0][key] = plan[key]
    return records, plan, reference


class FullForwardPreparationPolicyTests(unittest.TestCase):
    def test_both_variants_retain_packets_reference_and_timing_scope(self):
        for variant, transaction in check.VARIANTS.items():
            with self.subTest(variant=variant):
                report = check.validate_records(*fixture(variant))
                self.assertEqual(report["schema"], "FerricTargetFullForwardPreparationObservationV1")
                self.assertEqual(report["controller_cohort"], "full-forward-preparation-fence-v1-controller-v7")
                ...
                self.assertEqual(report["worker_source_manifest_sha256"], check.WORKER_SOURCE_MANIFEST_SHA256)
                self.assertIs(report["transaction_preparation_fence"], transaction)
    ...
    def test_metadata_adapter_never_mutates_inputs_or_existing_checker(self):
        values = fixture("transaction-checks")
    ...
    def test_setup_extras_are_exact_and_required_for_both_workers(self):
    ...
    def test_worker_variants_source_and_running_identity_are_not_interchangeable(self):
        for variant in check.VARIANTS:
            other = "transaction-checks" if variant == "per-dispatch-checks" else "per-dispatch-checks"
            for change_plan, change_setup in ((True, False), (False, True), (True, True)):
                records, plan, reference = fixture(variant)
                if change_plan:
                    plan["worker_sha256"] = check.WORKERS[other]
                if change_setup:
                    records[0]["worker_sha256"] = check.WORKERS[other]
                    records[0]["running_worker_sha256"] = [check.WORKERS[other]]
            ...
            for value in ("", "a" * 64, False):
                records, plan, reference = fixture(variant)
                plan["worker_source_manifest_sha256"] = value
                with self.subTest(variant=variant, source=value), self.assertRaises(ValueError):
                    check.validate_records(records, plan, reference)
            records, plan, reference = fixture(variant)
            records[0]["running_worker_sha256"] = [check.WORKERS[other]]
            with self.subTest(variant=variant, running_worker_only=True), self.assertRaises(ValueError):
                check.validate_records(records, plan, reference)
    ...
    def test_real_worker_identities_and_numeric_receipts_reach_core_unchanged(self):
        for variant in check.VARIANTS:
            ...
            self.assertEqual(observed_records[0]["running_worker_sha256"], [check.WORKERS[variant]])
            for key in batch.IDENTITIES:
        ...
        for variant in check.VARIANTS:
            for key, value in (
                ...
                with self.subTest(variant=variant, key=key, value=value), self.assertRaises(ValueError):
                    check.validate_records(records, plan, reference)
        records, plan, reference = fixture("transaction-checks")
