#!/usr/bin/env python3
"""Strict hidden-state RMSNorm comparison on a matched full-forward stack.
...
SOURCE_PINS = {
    "controller_source_manifest_sha256": "2489bee6d4237eeb5873487c96c67c37d0c44d91051b707b07dbfed5a87c3d6c",
...
CONTROLLER_SHA256 = "dbb09fb78689667c9c6bf80659770e9a340f1fbcbca4ff4520c9b94da3368545"
RMSNORM_ARTIFACT = {
...
PROFILE = "mfma-v3-fp32-v7-wave-attention-v11-v15-sidecars-tp1-616"
EXPECTATION_SCHEMA = "FerricTargetFullForwardRmsNormV15ExpectationV1"
STDERR = b"additional resident transposed weight bytes: 15136194560\n"
EXTRA_SETUP = {"runtime_full_forward", "full_forward_profile"}
VARIANTS = {"baseline": False, "wave-v15": True}
...
def load_pinned():
    ...
    try:
        ...
        os.close(descriptor)
    module = types.ModuleType("_frozen_full_forward_rmsnorm_v15_batch_checker")
...
core = load_pinned()
PLAN = core.PLAN | EXTRA_SETUP | HEAD_SETUP | ARGMAX_SETUP | RMSNORM_SETUP | set(SOURCE_PINS) | {
...
def checked_plan(plan):
    ...
    core.same(plan["runtime_full_forward"], True, "full-forward mode for both norm roots")
    ...
    core.same(plan["rmsnorm"], plan["variant"], "explicit norm mode")
...
def validate_records(records, plan, reference):
    ...
    extra = EXTRA_SETUP | ARGMAX_SETUP | RMSNORM_SETUP
    ...
    result.update(controller_cohort="full-forward-rmsnorm-v15-controller-v1", schema="FerricTargetFullForwardRmsNormV15ObservationV1", variant=plan["variant"],
    ...
    result["configuration"].update(runtime_full_forward=plan["runtime_full_forward"],
                                   ...
                                   fp32_argmax=plan["fp32_argmax"], rmsnorm=plan["rmsnorm"])
    result["nonclaims"].append(
        "Both variants load identical v11/v15 sidecars before allocation and retain the same controller, "
        "preparation-fence worker, paired-prefetch main image, FP32-v7 head and wave-attention/argmax. "
        "Only 73 pure width4096 RMSNorm symbols change; Q/K norms and all 616 packets remain. "
        "Wave reduction changes FP32 sum association while retaining both BF16 rounding boundaries.")
    result["nonclaims"].append(
...
def main(argv=None):
    ...
    try:
        ...
        parser.exit(1, f"Target full-forward norm-v15 observation rejected ({type(error).__name__}); no successful observation.\n")
    return 0
