#!/usr/bin/env python3
"""Strict MFMA/BF16 plus FP32-v7 full-forward submission observations.
...
PROFILE = "mfma-v3-fp32-v7-tp1-616"
EXPECTATION_SCHEMA = "FerricTargetFullForwardMfmaV7ExpectationV2"
STDERR = b"additional resident transposed weight bytes: 15136194560\n"
EXTRA_SETUP = {"runtime_full_forward", "full_forward_profile"}
VARIANTS = {"serial-mfma-v7-control": False, "full-forward-mfma-v7": True}
...
def load_pinned():
    ...
    try:
        ...
        os.close(descriptor)
    module = types.ModuleType("_frozen_full_forward_mfma_v7_batch_checker")
...
def checked_plan(plan):
    ...
    core.same(plan["performance_profile"], {
        ...
        "projection": "mfma", "attention": "baseline",
    }, "exact MFMA-v7 execution profile")
...
def validate_records(records, plan, reference):
    ...
    result.update(controller_cohort="rank-wrapper-fixed-v6", schema="FerricTargetFullForwardMfmaV7ObservationV2", variant=plan["variant"],
    ...
    result["nonclaims"].append(
        "Full-forward submission retains all 616 sequential packets per forward and unchanged MFMA/v7 arithmetic. "
...
def main(argv=None):
    ...
    try:
        ...
        parser.exit(1, f"Target MFMA-v7 full-forward observation rejected ({type(error).__name__}); no successful observation.\n")
    return 0
