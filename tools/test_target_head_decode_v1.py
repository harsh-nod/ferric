"""Synthetic strict head-checker tests; not GPU or numerical evidence."""
...
import target_head_decode_v1 as check
from test_target_batch_decode_v1 import fixture as base_fixture


def fixture(variant="baseline-fp32-v7"):
    records, plan, reference = base_fixture(device_residual=False)
    projection, precision = check.VARIANTS[variant]
    plan.update(schema="FerricTargetHeadDecodeExpectationV1", variant=variant, kernel_profile="v3-mfma",
...
class TargetHeadPolicyTests(unittest.TestCase):
    def test_exact_three_variants_keep_full_reference_and_dispatches(self):
        for variant in check.VARIANTS:
            with self.subTest(variant=variant):
                ...
                self.assertEqual(report["schema"], "FerricTargetHeadDecodeObservationV1")
    ...
    def test_incoherent_or_extended_expectations_rejected(self):
        for key, value in (("schema", "FerricTargetBatchDecodeExpectationV1"), ("variant", "mfma-bf16-v7-control"),
                           ...
                           ("collective", "device-tp1-v3"), ("host_timing_enabled", True),
    ...
    def test_runtime_flags_and_projection_are_not_normalized_before_validation(self):
        for key, value in (("runtime_cache_admission", False), ("runtime_operational", False),
                           ...
                           ("runtime_profiling", True), ("projection", "mfma"), ("attention", "wave")):
            records, plan, reference = fixture()
    ...
    def test_common_schedule_bytes_timing_and_closure_checks_still_apply(self):
        for index, key, value in ((0, "dtype", "FP8"), (0, "prefix_cache", True),
                                  (0, "output_head_pruning", True), (2, "rank_dispatch_counts", [616]),
    ...
    def test_compare_pins_reference_status_workload_and_capture(self):
        ...
        with self.assertRaisesRegex(ValueError, "reference digest"):
            check.compare(capture, b"0\n", workload, json.dumps(reference).encode(), expected)
    ...
    def test_cli_rejection_does_not_create_output(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            arguments = [arg for key in ("capture", "status", "workload", "reference", "expect")
