#!/usr/bin/env python3
"""Strict 32-token MFMA/device-TP1 v7-head attention comparison.

All weights and intermediate activations remain BF16. The explicitly selected
output head stores FP32 logits. Both attention implementations must reproduce
...
def load_shared_checker():
    ...
    module = types.ModuleType("_target_v7_wave16_frozen_batch_checker")
...
CONTROLLER_SHA256 = "148e6226c57f155334db7858211a11ed842cb4dfce8de2c991ec5f0b7d483ef4"
...
VARIANTS = {"mfma-device-baselineattention": "baseline", "mfma-device-waveattention": "wave"}
...
def checked_plan(plan):
    ...
    batch.same(plan["schema"], "FerricTargetV7Wave16ExpectationV1", "head expectation schema")
    batch.require(type(plan["variant"]) is str and plan["variant"] in VARIANTS, "head variant")
    attention = VARIANTS[plan["variant"]]
    precision = "fp32-v7"
    batch.same(plan["worker_sha256"], WORKER_SHA256, "frozen old runtime worker")
    batch.same(plan["controller_sha256"], CONTROLLER_SHA256, "frozen v7 wave16 controller")
    ...
    batch.same(plan["collective"], "device-tp1-v3", "head collective")
    ...
    batch.same(plan["fp32_head_workspace_bytes"], 9_723_904,
    ...
    batch.same(profile["projection"], "mfma", "head projection variant")
    batch.same(profile["attention"], attention, "explicit attention variant")
...
def validate_records(records, plan, reference):
    ...
    result["configuration"]["kernel_row_capacity"] = 16
    ...
    result["nonclaims"].append(
        "This distinct v7 profile keeps MFMA projection and device TP1 residuals fixed; only attention is selected. "
        "FP32 logits are explicit. This does not qualify wave projection, ordered MFMA or a different model prompt.")
    return result


def compare(capture, status, workload, reference, expectation, stderr):
    batch.require(status == b"0\n", "successful controller status")
    batch.require(stderr == STDERR, "exact unprofiled transposed-weight preamble")
    ...
    result["input_sha256"] = {"capture": batch.sha256(capture), "status": batch.sha256(status),
                             ...
                             "expectation": batch.sha256(expectation), "stderr": batch.sha256(stderr)}
    return result
...
def main(argv=None):
    ...
    try:
        report = compare(batch.read_bounded(args.capture, 1024 * 1024), batch.read_bounded(args.status, 16),
                         ...
                         batch.read_bounded(args.expect, 65536), batch.read_bounded(args.stderr, 65536))
        ...
        parser.exit(1, f"Target v7 wave16 observation rejected ({type(error).__name__}); no successful observation.\n")
    return 0
