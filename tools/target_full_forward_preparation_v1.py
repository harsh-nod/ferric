#!/usr/bin/env python3
"""Strict preparation-currentness worker comparison with identical full-forward execution.
...
WORKERS = {
    "per-dispatch-checks": "8284a44f702b8e42a35fe9e04cf051dd709ce9995329adaa2ea5a57ab80fb06a",
    "transaction-checks": "05503764710e8add6ee15fbb1f8526ed9bdddfce7dd670acd7c604f9491490a7",
}
...
EXPECTATION_SCHEMA = "FerricTargetFullForwardPreparationExpectationV1"
STDERR = b"additional resident transposed weight bytes: 15136194560\n"
EXTRA_SETUP = {"runtime_full_forward", "full_forward_profile"}
VARIANTS = {"per-dispatch-checks": False, "transaction-checks": True}
...
def load_pinned():
    ...
    try:
        ...
        os.close(descriptor)
    module = types.ModuleType("_frozen_full_forward_preparation_batch_checker")
...
core = load_pinned()
PLAN = core.PLAN | EXTRA_SETUP | HEAD_SETUP | {"variant", "common_comparator_sha256", "worker_source_manifest_sha256"}
...
def checked_plan(plan):
    ...
    core.same(plan["runtime_full_forward"], True, "full-forward mode for both workers")
    core.same(plan["full_forward_profile"], PROFILE, "full-forward wave profile")
    core.same(plan["worker_source_manifest_sha256"], WORKER_SOURCE_MANIFEST_SHA256, "frozen preparation worker source")
    ...
    core.same(plan["worker_sha256"], WORKERS[plan["variant"]], "frozen worker for this variant")
...
def validate_records(records, plan, reference):
    ...
    result.update(controller_cohort="full-forward-preparation-fence-v1-controller-v7", schema="FerricTargetFullForwardPreparationObservationV1", variant=plan["variant"],
                  ...
                  transaction_preparation_fence=VARIANTS[plan["variant"]],
    ...
    result["nonclaims"].append(
        "Both workers retain all 616 sequential packets per forward, 36 completion frontiers, "
        "and identical MFMA/v7/wave-attention images and runtime flags. "
        "Only the worker identity and preparation-currentness policy vary; currentness check counts are not measured.")
    result["nonclaims"].append(
        "The transaction preparation fence retains initial, periodic, stage/publication and completion currentness checks. "
        "This comparison does not claim a persistent GPU kernel, overlap, removed correctness checks or a qualified speedup.")
    return result
...
def main(argv=None):
    ...
    try:
        ...
        parser.exit(1, f"Target preparation-fence full-forward observation rejected ({type(error).__name__}); no successful observation.\n")
    return 0
