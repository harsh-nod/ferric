window.FERRIC_PROJECT = Object.freeze({
  ...
  residentCheckpoint: Object.freeze({
    ...
    overview: "Ferric now adopts published compiler 1fadb7e0 after 47 dependency/source-gate phases; this is not new engine, proof or native validation. The combined M5 capture source passes 750 engine tests, 22 smoke tests and three Python suites on earlier compiler 349b2cd0. Its fresh proof verifies one production catch-up method, not the whole engine; nine negative mutations remain attributed to compiler 111. The retained compiler-111 run completes 32 tokens, while its separate prefill comparison remains numerically unqualified. The core compiler correction's original R5 campaign passes 52 selected tests, but strict Clippy exits 101 at 17 unchanged sites. No new M5 native capture or reference result is claimed. MFMA13 remains opt-in, with LegacyScalar12 the default. No new TTFT, TPOT, throughput or serving qualification. All 33 M1 gates remain open.",
    ...
    remaining: "Next: build the current seven-case capture executable, run the remaining R29 GPU cases, finish resident validation and exercise repeated rounds on hardware. Latest combined host validation is separate from the retained bb2/ccfd cohorts below. The selected logit comparison accepts no tolerance; neither target-only observation qualifies resident speculative serving. No native K8/K16 qualification, new TTFT/TPOT/throughput measurement or competitiveness claim is admitted here. All 33 M1 gates remain open. Historical measurements below are unchanged.",
  }),
