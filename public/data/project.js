window.FERRIC_PROJECT = Object.freeze({
  ...
  residentCheckpoint: Object.freeze({
    ...
    overview: "Private draft KV catch-up is integrated and host-tested. The retained c5/4f6 engineering run completes five real K4 rounds and eight tokens with zero/partial acceptance; full-acceptance catch-up/restore is not observed. Longer attempts expose generation bookkeeping and then maintenance publication failures. Frozen 00ac diagnostics pass scoped host checks and an optimized build, but R5 stops before inference on a process-observation race; no maintenance result is inferred. Helper integration 8fa40b3a passes eight remote host phases and 723 engine tests with nine ignored; six actual engine helper bodies have separate scoped proof results. Whole-roster/native composition and successor source coverage remain pending; the earlier c5 source coverage is historical. Frozen e0/ce2 now emits and exactly replays the thirteen-root Ferric MFMA image, with no GPU execution or numerical result. Tested private compiler 86ccc2a5 on observed base bd4d5f42 passes 23 delta host phases; its release tools and emission are not claimed. Newer observed upstream 48323569 remains unvalidated. The retained 0fe 12-kernel image is separate; that image is not GPU-tested. Actual GPU comparisons remain 1 of 7. The retained numerical rows are not byte-identical and no tolerance is accepted. All 33 M1 gates remain open; no new serving or performance result is claimed.",
    integration: Object.freeze({
      ...
      compilerFollowup: Object.freeze({
        ...
        newerObservedUpstreamSource: "483235698d6603e2a0140ecad5932d914575dbe5",
        newerUpstreamValidated: false,
        ...
        published: false,
        detail: "The retained 5602 row-major and column-major compiler fixtures emit 12,208-byte and 12,048-byte gfx942 images with exact replay and inspected BF16 MFMA instructions. These are fixtures, not the Ferric aggregate or GPU execution. The earlier ce2/5602 aggregate stops on unresolved loop-header control dependence before artifact emission. Tested private compiler 86ccc2a5 retains all twelve unchanged patches on observed base bd4d5f42. Its R9 remote delta host campaign completes all 23 phases with exit 0, covering backend, lowerer, Pliron, tile mapping, frozen MIR V13/V14, macro closure and the paired BF16 fixture, with actual artifact freshness checks. Subsequently observed public main 48323569 has not been rebased into this candidate or host-validated. New 86 source-bound tool builds have not run; this host result is not new emission, GPU execution or qualification. Frozen compiler e0d108b2 separately passes 42 host phases and four actual tool build/copy phases. Its ce2 thirteen-root campaign passes all 13 preparation, emission, after-check and inspection phases: the 113,192-byte gfx942 image exactly replays, has 13 entry/descriptor pairs and contains v_mfma_f32_16x16x16_bf16 in the MFMA GEMM kernel. The complete emission archive is retained and SHA-256 checked. These e0 tools and image are not relabeled as 86; no GPU ran, and publication/load/launch grants remain false. The seventeen passing host phases belong to predecessor d3 on bf0 and remain separate; its release rebuild stopped at the unchanged root-disk floor, so no d3 emission is claimed. The separately host-tested 5df/5602 numerical probe has ten tests, 39 policies and a checked 108-case CPU listing, but no GPU numerical execution. No numerical result, performance gain, qualification or compiler publication follows from these host and emission records.",
      }),
