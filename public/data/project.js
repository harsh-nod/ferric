window.FERRIC_PROJECT = Object.freeze({
  ...
  residentCheckpoint: Object.freeze({
    ...
    overview: "Private draft KV catch-up is integrated and host-tested. The retained c5/4f6 engineering run completes five real K4 rounds and eight tokens with zero/partial acceptance; full-acceptance catch-up/restore is not observed. Longer attempts expose generation bookkeeping and then maintenance publication failures. Frozen 00ac diagnostics pass scoped host checks and an optimized build, but R5 stops before inference on a process-observation race; no maintenance result is inferred. Integrated index source 21818e2a passes eight remote host phases and 724 engine tests with nine ignored; six actual engine helper bodies and the index core have separate scoped proof results. Seven actual index-body negatives are retained. Ledger candidate efa069bd passes 726 engine tests with nine ignored but fails strict engine Clippy; corrected 47d07cee's original host run is recovered with exit 0, 726 engine tests with nine ignored and strict engine/spec Clippy passed. Its full host evidence is retained. R9 proves nine actual helper bodies, and R10 completes six actual ledger-body negative checks; both proof archives are retained and independently reviewed. The verified ledger helpers are integrated at 66e32993 with code bytes equal to source 47d07cee; host and proof receipts remain attributed to 47. Whole-roster/native composition and successor source coverage remain pending; the earlier c5 source coverage is historical. Frozen e0/ce2 now emits and exactly replays the thirteen-root Ferric MFMA image, with no GPU execution or numerical result. Tested private compiler 86ccc2a5 on observed base bd4d5f42 passes 23 delta host phases; its release tools and emission are not claimed. Published compiler e3c359fb is rebased onto observed upstream 2585ce64; R12 completes all 19 mi300x phases: 1,030 Rust tests pass with one existing gfx1151 ignore, plus nine Python dependency tests and the CI dispatch harness. Earlier harness and retainer failures remain separate; the full R12 archive is locally retained and independently audited. A normal compiler-main push of e3c359fb is confirmed. Private integration 9e10cc95 now pins published e3c359fb through an 80-file dependency refresh. The 588 dependency-only overlay passes 42 remote phases, 32 locked/offline graphs and 38 source-gate tests; the newer 8af contracted batch ledger consumer loop remains unvalidated. Frozen native 00ac/4f6 and source47 receipts retain their identities. Previous candidate 12845295 on 48323569 passes all 14 retained R10 delta host phases. No 128 release tools, emission, strict compiler Clippy, GPU result, push or Ferric repin is claimed. Read-only mi350-2 inventory finds one gfx950 GPU, foreign KFD queues and an unadmitted kernel; no launch occurred. The retained 0fe 12-kernel image is separate; that image is not GPU-tested. Actual GPU comparisons remain 1 of 7. The retained numerical rows are not byte-identical and no tolerance is accepted. All 33 M1 gates remain open; no new serving or performance result is claimed.",
